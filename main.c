...
void init_disk() {
    ...
    Inode root_inode = make_inode(DIR, 0);  // Assuming root is a directory with size 0
...
int main() {
     ...
     FILE *file = fopen("disk_log.bin", "wb");
    if (file == NULL) {
        perror("fopen");
        exit(1);
    }
    ...
    fwrite(disk, sizeof(Block), disk_len, file);

    // Close the file
    fclose(file);
    FILE *file1 = fopen("disk_log.bin", "rb");  // 'rb' for reading binary
